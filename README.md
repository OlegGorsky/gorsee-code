...
Для очевидных маленьких файловых задач вроде `создай файл smoke.txt с текстом
hello` используется ускоренный coding path: `coder` и `validator` без отдельного
архитектурного прохода, с пониженным reasoning и меньшим бюджетом. Более
сложные `refactor`, `fix`, `implement`, `review` и `release` запросы идут через
полный жизненный цикл планирования, tools, diff и проверок.

### Coding core

TUI, CLI и ACP работают поверх одного ядра:
...
Внутренний LCP-контракт хранит intent, plan, execution steps, transcript, diff,
verification, approvals и usage. Клиенты получают уже очищенное состояние:
сырой `tool_requested`, `tool_finished`, JSON payloads и длинный stdout/stderr не
отображаются как сообщения чата.
...
- user-level auth файла `~/.config/gorsee-code/auth.json`.

Если ключ введен при первом запуске TUI, уже найден в локальном workspace или
сохранен через `gcode auth set`/`gcode setup`, он также зеркалируется в
user-level auth, чтобы новые рабочие папки не просили ключ повторно.
...
CLI-вывод `gcode exec` печатает actionable id в строке вида
`approval: appr_0009 требуется подтверждение: apply_patch`.
...
| `gcode mcp` | показывает найденные MCP-серверы, MCP tools и локальные bridge tools |
...
MCP-конфиги читаются из проекта и пользовательских папок: `.mcp.json`,
`.cursor/mcp.json`, `.codex/mcp.json`, а также user-level конфиги. MCP calls
идут через общий tool runtime и safety/approval слой.

### ACP

ACP surface нужен для внешних клиентов и использует тот же `CodingOrchestrator`,
что TUI и CLI:

```bash
gcode acp
gcode acp plan "создай файл smoke.txt с текстом hello"
gcode acp run "проверь diff"
gcode acp stdio
```

Adapter маппит внешние session/turn в Gorsee workspace/session/turn и стримит
progress, approvals, diff, verification и final answer без дублирования
агентной логики.
